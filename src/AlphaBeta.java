...
public class AlphaBeta
{
	...
	private static int minimax(Board board, int levels, boolean isMax, String playerType, int alpha, int beta, String currentPlayer) {
		String opponent = (Objects.equals(currentPlayer, "AI")) ? "Human" : "AI";
...
// return score when achieve max level
		if (levels == maxLevel) {
			if (board.countFours(playerType) > board.countFours(opponent)) {
				return score * board.countFours(playerType) * (maxLevel);
			} else if (board.countFours(playerType) < board.countFours(opponent)) {
				return score * board.countFours(opponent) * (maxLevel);
			} else {
				return score;
			}
		}
		...
		if(isMax) {
			...
			for(int i = 0; i < Board.COLUMNS; i++) {
				if(board.columnsCounter[i] > 5) { // columns are full
					continue;
				}
				...
				int value = minimax(board, levels + 1, false, playerType,alpha,beta,currentPlayer);
