	...
	private static int minimax(Board board, int depth, boolean isMax, String playerType, String currentPlayer) {
		...
		if (depth == maxDepth) {
			if (board.countFours(playerType) > board.countFours(opponent)){
				return score * board.countFours(playerType) * (maxDepth);
			}
			else if (board.countFours(playerType) < board.countFours(opponent)) {
				return score * board.countFours(opponent) * (maxDepth);
			} else {
				return score;
			}
		}
		...
		if (isMax) {
			...
			for(int i = 0; i < Board.COLUMNS; i++) {
				...
				best = Math.max(best, minimax(board, depth + 1, false,playerType,currentPlayer));
		...
		// If this minimizer's move
			...
			for(int i = 0; i < Board.COLUMNS; i++) {
				...
				best = Math.min(best, minimax(board, depth +1, false,playerType,currentPlayer));
